...
>* **The WonderMind module supports voice large model calls for the “Volcengine” and “Aliyun” platforms. This tutorial only shows the TTS (Text-to-Speech) functionality of the international version of the “Aliyun” voice large model.**
...
| `tts_model`  | `string`       | Sets the TTS model, supports "aliyun" and "volcengine"                    |
...
| `access_token`  | `string`       | Sets the access token for TTS service authentication (required only for Volcano TTS service) |
...
| ---------- | --------------------------------------------- | -------- | ---------- |
| `Chinese-English Female`           | `zh_female_shuangkuaisisi_emo_v2_mars_bigtts` | Volcengine          | Chinese, English |
| `Guangxi Yuanzhou`           | `zh_male_guangxiyuanzhou_moon_bigtts` | Volcengine          | Chinese |
| `Cute Younger Female` | `zh_female_yuanqinvyou_moon_bigtts`           | Volcengine          | Chinese |
...
{
    "command": "tts_model",
    "params": "aliyun", // or "volcengine"
}
```

`params`: The TTS model name, supports **aliyun** and **volcengine**, but cannot be used simultaneously.
...
{
    "command": "voice",
    `params`: "Chinese-English Female"
}
```

① `params`: Voice tone name, using the names from the voice tone list.

② For a detailed voice tone list of Volcengine, refer to: [Volcengine Voice Tone List](https://www.volcengine.com/docs/6561/1257544)
...
(1) Set the TTS model, either aliyun or volcengine. They cannot be used simultaneously.
...
* **Preparation**
> [!note]
> **Before learning this section's example, please ensure that you have completed studying [“2.1 I2C Communication Protocol ”](#anther2.1).**
...
Open "**Appendix\01 MicroPython Program Files\main.py**", and enter the user information obtained from configuring the online speech model platform in the code below. Then click save and close.
...
if use_aliyun:
  ...
  control.send_message({"command":"app_id","params":""})         
  # Enter the API_KEY created on the Volcengine platform here.
...
> * **If using ESP32, make sure that the MicroPython firmware is already installed on the ESP32 before downloading MicroPython. For firmware downloads, please visit the MicroPython official website. If using Hiwonder ESP32 core board, flash the firmware file provided by Hiwonder (path: **Appendix\03 ESP32 Core Board Firmware and Flashing Tool\02 ESP32 Firmware Flashing Tool**.) For flashing instructions, please refer to the document in the same path.**

1. Open the **Hiwonder Python Editor** software <img src="..\_static\media\chapter_2\section_2/media/image1.png"/> (path: **Appendix\02 Hiwonder Python Editor**);

Drag the “**Appendix\01 MicroPython Program Files\main.py**” file, located in the same directory as this document, into the Hiwonder Python Editor. Ensure it is dragged into the red-framed area for it to take effect.
...
Call the voice model from Alibaba Cloud/Volcengine platform and synthesize the announcement of "Hello, how can I assist you?" in various language versions.
...
```python
status = 'tts_test'
use_aliyun = True  # Set to True to use the Aliyun TTS model (True - Aliyun, False - Volcengine)
...
if use_aliyun:
  ...
  control.set_tts_model("volcengine")
  control.send_message({"command": "app_id", "params": ""})  # Insert the app_id created on the Volcengine platform here
  control.send_message({"command": "access_token", "params": ""})  # Insert the access_token created on the Volcengine platform here
```
...
**Path: Appendix\01 MicroPython Program Files\main.py**
...
1. Please open the **flash_download_tool_3.9.7.exe** file located in the "**Appendix\03 ESP32 Development Board Firmware and Flashing Software\02 ESP32 Firmware Flashing Tool**" folder.
...
3. Once the tool is open, click "**...**" to select the program .bin file you want to flash (path: [“03 ESP32 Development Board Firmware](https://drive.google.com/drive/u/0/folders/1RNE6zbiYoPGZ6WH_1pAVhFYykR1SQ5Qw)).
...
> [!note]
> **Note: Before learning this section's example, please ensure that you have completed studying [“2.1 I2C Communication Protocol ”](#anther2.1).**
...
(2) Open the **"Appendix\04 Arduino Program Files\01 MCP Tool Usage\WonderLLM_example"** program file in the document path.
...
(4) Click <img src="../_static/media/chapter_2/section_3/media/image3.png" style="width:40px" /> to download the program to the development board and wait for the download to complete.
...
Open "**Appendix\04 Arduino Program Files\01 MCP Tool Usage\WonderLLM_example**", fill in the user information obtained from the online speech model platform as described earlier, and then click save and close.

```c
const char TTS_model_uesr_app_id_aliyun[] PROGMEM = "";           
// Enter the API_KEY created on the Alibaba Cloud platform here.
const char TTS_model_uesr_app_id_volcengine[] PROGMEM = "";             // Enter the app_id created in the Volcengine platform here
const char TTS_model_uesr_access_token_volcengine[] PROGMEM = "";      // Enter the access_token created in the Volcengine platform here    
// Enter the access_token created on the Volcengine platform here.
```
...
(5) In the software interface, click the **File** button, then select the "**Appendix\05 STM32 Program Files\01 MCP Tool Usage\RosRobotControllerM4-armclang\MDK-ARM\RosRobotControllerM4\RosRobotControllerM4.hex**" file for flashing.
...
> [!note]
>
>* this function is ineffective before the module completes the network configuration. Ajust the 18-second delay in the program according to the actual network configuration time of the module.
>* The string can be customized. Please ensure that the meaning is concise and highlights the recognition purpose and returned content.
...
(1) Open "**Appendix\05 STM32 Program Files\01 MCP Tool Usage\RosRobotControllerM4-armclang\Hiwonder\System\app.c**", fill in the user information obtained from the online speech model platform as previously configured, then click save and close.

```c
const char TTS_model_uesr_app_id_aliyun[] = "";           
// Enter the API_KEY created on the Alibaba Cloud platform here.
const char TTS_model_uesr_app_id_volcengine[] = "";             // Enter the app_id created in the Volcengine platform here
...
// Voice models from Volcengine
const char spokesperson_zhong_ying_nv[] = "zh_female_shuangkuaisisi_emo_v2_mars_bigtts"; //Chinese-English Female
const char spokesperson_guang_xi_yuan_zhou[] = "zh_male_guangxiyuanzhou_moon_bigtts";    //Guangxi Yuanzhou
const char spokesperson_sa_jiao_xue_mei[] = "zh_female_yuanqinvyou_moon_bigtts";         //Cute Younger Female
...
(1) Drag the program files from the **"Appendix\07 microbit Program Files"** path into the MakeCode page.

<img src="..\_static\media\chapter_2\section_5\media\image3.png" style="width:5.76389in;height:3.82222in" />

(2) Click the icon at the lower left of the interface <img src="..\_static\media\chapter_2\section_5\media\image4.png" style="width:0.31319in;height:0.29653in" />, then click **Connect Device** and follow the on-screen instructions to connect to the micro:bit controller.
...
<img src="..\_static\media\chapter_2\section_5\media\image11_1.png" style="width:500px"  class="common_img" />
...
<table>
<table><tr><td>Supported Features</td><td>WonderLLM.bin firmware for secondary development by Hiwonder</td><td>Hiwonder_xiaozhi.bin official firmware</td></tr>
...
>* **For the flashing method, use the corresponding flashing software as described in “1. Basic Course / 1.8 Flashing Firmware.” Flash the hiwonder_xiaozhi.bin (path: “1. Basic Course\Appendix\05 ESP32S3 Firmware\01 Official Open Source Firmware”) to the module in the same manner. 
This section will not cover this process. The following text will specifically introduce the procedure for compiling and downloading the project files to the module.**
...
2) Select the XiaoZhi open-source program project folder (path: **Appendix\06 XiaoZhi Open-Source Program Project**) and open it.
> [!note]
...
**Path: Appendix\06 XiaoZhi Open-Source Program Project**
