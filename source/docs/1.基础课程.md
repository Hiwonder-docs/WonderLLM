...
3. 模块支持唤醒词修改和自定义词条，可前往“[**1.7 唤醒词修改**](#anther1.7)”学习如何修改。

4. 教程均以中文识别为例，如需烧录英文固件，可前往[**“1.8 固件下载”**](#anther1.8)内查看固件烧录教程。
...
>[!note]
>
>**WonderLLM模块提供中文、英文2个系统语言版本的出厂固件供使用，如用户开机后发现模块运行的出厂固件系统语言不匹配需求，可以直接跳转至 “[1.10.3 CI1302固件烧录方法](#p1-10-3)” 和 “[1.11.1 固件下载](#p1-11-1)” ，分别烧录对应语言版本的2个固件。**


1)  模块支持如下接口供电：**①Type-C接口（上方）**、**②Type-C接口（下方）**  **③4-PIN IIC通信接口**，用户可任选一处接口，接入外部供电设备，模块供电自动开机。

2. 模块上电后，若为第一次使用，则画面首先显示**WIFI名称**及**浏览器配网网址**并伴随语音播报，用户需要按照[“1.2 联网及配置”](#anther1.2)完成模块设备配网。

<img src="..\_static\media\chapter_1\section_1\media\image3.png"     class="common_img"  style="width:400px"  class="common_img"/>


3. 模块配网完成后需要完成与平台智能体绑定，模块画面显示**设备ID**及**设备绑定平台网址**并伴随语音播报绑定验证码，用户需要按照[“**1.2 联网及配置**”](#anther1.2)完成模块设备绑定。

<img src="..\_static\media\chapter_1\section_1\media\image4.png"     class="common_img"  style="width:400px"  class="common_img"/>


4. 模块上电后，若已完成设备绑定，屏幕显示白色环形滚动条，此时模块正在根据内部存储WIFI信息自行联网。若模块联网失败，则画面显示**设备热点名**及**设备访问网址**并伴随语音播报，用户需要按照[“**1.2 联网及配置**”](#anther1.2)再次配置当前环境可用的新WIFI连接信息。
...
3)  若用户通过长按右侧按键重启，若模块重新开机后尚未松开按键，可能导致模块误判断进入配网模式（详见**[“1.2 联网及配置”](#anther1.2)**），带来不必要的麻烦，因此推荐通过长按左侧按键实现模块重启功能。
...
2. 用户可点击任意一条已保存的热点连接信息对应的<img src="..\_static\media\chapter_1\section_2\media\image10.png" style="width:70px" />图标，将其删除，也可点击<img src="..\_static\media\chapter_1\section_2\media\image11.png" style="width:70px"  />图标提升其在模块配网时匹配次序优先级(列表由上到下优先级递减)。
...
5. 若用户初次登录，需要使用注册平台账号，账号注册流程此处不再展开。

<img src="..\_static\media/chapter_1/section_2/media/image25.png"     class="common_img" style="width:600px"/>

6. 点击右上角的下拉按键，并点击”新建智能体“。

<img src="..\_static\media/chapter_1/section_2/media/image26.png"     class="common_img" style="width:600px"/>

7. 新建一个新的智能体，同时输入对应智能体名字（可以任意填写），并点击确认。

<img src="..\_static\media\chapter_1\section_2\media\image27.png"     class="common_img" style="width:600px"/>

8. 点击**“配置角色”**。

<img src="..\_static\media\chapter_1\section_2\media\image16.png"     class="common_img"/>

9. 在配置界面中，①处助手昵称填写“**小幻**”(**不推荐修改，如需修改还需参照[1.7 唤醒词修改](anther1.7)同步修改模块唤醒词**)，②处对话语言、角色音声可根据自身需求选择，③处角色介绍可使用默认模板或自定义(**此处推荐使用我司提供的如下角色介绍，以便智能体互动时更好适应后续玩法对话**)。
...
10. 记忆体类型选**“无记忆”**，语言模型项推荐选择“**DeepSeek V3(性格丰富)**”，高级设置项可调节更多详细工作参数，此处保持默认设置，最后点击**"保存"**即可。

<img src="..\_static\media\chapter_1\section_2\media\image28.png"     class="common_img"/>

<img src="..\_static\media\chapter_1\section_2\media\image18.png"     class="common_img"/>

11. 保存成功后，返回上一级界面， 点击“**添加设备**” 。

<img src="..\_static\media\chapter_1\section_2\media\image19.png"     class="common_img"/>

12. 在弹窗输入6位设备ID，点击确定，即完成设备绑定 。

<img src="..\_static\media\chapter_1\section_2\media\image20.png"     class="common_img"/>

13. 若绑定成功，可在页面观察到“**添加设备成功**”提示，如下图 。选择“**开源版**”，点击“**开始使用**”。
...
>[!note]
>**在进行本节学习前，请确保已根据[“1.1 WonderLLM介绍”](#anther1.1)、[“1.2 联网及配置”](#anther1.2)打开模块并完成WIFI、智能体配置。**
...
>[!note]
>
>**表情界面即为模块上电并联网成功后出现的界面；聊天界面为用户说出唤醒词/短按右侧按键进入的界面，会在[1.4 聊天模式](#anther1.4)详细介绍。**
...
2. 模块若干模式的切换关系如下图。可将**进入聊天模式**的操作总结如下：在其他任意模式下，通过**说出唤醒词/短按右侧按键**(详见[**1.4.2 操作说明**](#anther1.4.2))， 即可进入聊天模式。
...
3. 如果以**说出唤醒词方式**进入聊天模式，模块识别到唤醒词后**默认以用户身份发送“你好”**作为对话开场语，大模型会**做出恰当回复**显示在聊天框界面并同步播报(可被人为打断从而快速跳过，具体操作详见[“**1.8 自由聊天**”](#anther1.8))，**之后才开始侦听**用户发言。
...
5. 进入聊天模式，开始人机交互后，用户可以选择让模块**不显示聊天框界面**，而是**持续保持表情界面**，具体操作方式详见[**1.5.2 操作说明**](#anther1.5.2)。
...
4. 在摄像头模式下，单击屏幕任一位置可触发场景理解功能(详见[“**1.9 场景理解**”](#anther1.9))。
...
<img src="..\_static\media\chapter_1\section_3\media\image7.png"     class="common_img"  style="width:400px"  class="common_img"/>
...
8. 接下来填写项目的基本信息，我们需要识别英文，那么语言类型选择“**英文**”（**识别中文只需要“语音类型”选择及“选择声学模型”选为中文即可）**，其余信息按照下图选择，完成后点击“**继续**”。
   ...
   <img src="../_static\media/chapter_1/section_7/media/imagea_2.png"   class="common_img" />
...
<img src="../_static\media/chapter_1/section_7/media/imageb_2.png"   class="common_img" />

6. 下面进入到编辑命令词功能中，首先需要选择播放的音色，这里我们选择“**Dane-英语男声**”。**（如果是中文的话也可以选择程程-标准男童 Ver.1）**

>[!note]
>
>* **此处的音色并不是WonderLLM模块与用户交互发言时的音色(如需调节，请前往小智AI平台控制台，在绑定当前模块设备ID的智能体中调节，详见[1.2 联网及配置](#anther1.2)，之后重启设备即可生效)。**
...
<img src="../_static\media/chapter_1/section_7/media/imageb3.png"   class="common_img" />
...
<img src="../_static\media/chapter_1/section_7/media/imageb4.png"   class="common_img" />
...
<img src="../_static\media/chapter_1/section_7/media/image19.png"   class="common_img" />

<img src="../_static\media/chapter_1\section_7/media/image20.png"   class="common_img" />



### 1.10.2 命令词播报词协议列表

[1.10.2 命令词播报词协议列表](https://pan.baidu.com/s/1dEzSQH2UxW_UqMbu0DZ-Dg?pwd=upx2)
...
>[!note]
>**我们以烧录 “附录\03 CI1302出厂固件” 路径下的出厂固件“hello hiwonder固件（唤醒词：hello hiwonder）”为例，操作步骤也一致适用于中文唤醒词烧录。**
...
[1.10.4 CI1302固件烧录工具](https://pan.baidu.com/s/1dEzSQH2UxW_UqMbu0DZ-Dg?pwd=upx2)



### 1.10.5 出厂固件

[1.10.5 出厂固件](https://pan.baidu.com/s/1dEzSQH2UxW_UqMbu0DZ-Dg?pwd=upx2)
...
>[!note]
>
>* **固件重新烧录后，需要重新配置WIFI连接信息，操作方法请见[“1.2 联网及配置”](#anther1.2)。**
...
1. 请打开[1.11.2 ESP32固件烧录工具\flash_download_tool_3.9.7](https://pan.baidu.com/s/1dEzSQH2UxW_UqMbu0DZ-Dg?pwd=upx2)压缩包里面的**flash_download_tool_3.9.7.exe**文件。
...
3. 工具打开后，点击**“...”**选择需要烧录的程序bin文件，下面以我司出厂固件WonderLLM.bin（存放路径：**“附录/05 ESP32S3出厂固件”** ）为例：
...
[1.11.2 ESP32固件烧录工具](https://pan.baidu.com/s/1dEzSQH2UxW_UqMbu0DZ-Dg?pwd=upx2)



### 1.11.3 ESP32S3固件

[1.11.3 ESP32S3固件](https://pan.baidu.com/s/1dEzSQH2UxW_UqMbu0DZ-Dg?pwd=upx2)
