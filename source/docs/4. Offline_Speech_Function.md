...
>* **For detailed explanations of the "Command Word Broadcast Protocol List", please refer to [4.10 Command Word Broadcast Speech ID Analysis](#4.10).** 
...
>
> * **The internal I2C device address switch requires some time. If a command is sent to the WonderLLM module immediately after switching to the new I2C address, the module will not respond correctly.
 To ensure proper communication, please wait at least 100ms before attempting to communicate with the module using the new address.**
...
> [!NOTE]
>
>**The WonderLLM module STM32 example runs on the FreeRTOS system, with the app_task task provided to execute custom logic. The main.c file is only used for initializing system peripherals and starting the RTOS system. 
